...
from splat_annotate.camera.transforms import project_points, unproject_pixels
...
def resize_shortest(image_size:np.ndarray, 
                    min_size:int, max_size:int|None=None) -> tuple[np.ndarray, float]:
  ...
  new_size = np.round(image_size * scale).astype(np.int32)
...
class FOVCamera:
  ...
  def resize_shortest(self, min_size:int, max_size:int|None=None) -> 'FOVCamera':
  ...
  def with_pose(self, pose:np.ndarray) -> 'FOVCamera':
    rotation, position = split_rt(pose)
    return replace(self, rotation=rotation, position=position)
  ...
  def unproject_pixel(self, xy:np.ndarray, depth:float) -> np.ndarray:
    points = self.unproject_pixels(xy, np.array([[depth]]))
    return points[0]
  
  def project_point(self, x:float, y:float, z:float) -> np.ndarray:
    xy, depth = self.project_points(np.array([[x, y, z]]))
    return np.array([*xy[0], *depth[0]])
