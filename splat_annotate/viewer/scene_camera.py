...
from splat_annotate.camera.fov import FOVCamera, join_rt
...
class SceneCamera:
  ...
  def look_at(self, pos:np.ndarray, target:np.ndarray, up:np.ndarray=np.array([0, 0, 1])):
    self._camera = self._camera.with_pose(look_at_pose(pos, target, up))


  def resized(self, size:np.ndarray):
