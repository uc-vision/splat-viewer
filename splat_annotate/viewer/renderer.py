...
from splat_annotate.gaussians import Gaussians, Rendering
from splat_annotate.viewer.scene_camera import to_pyrender_camera
...
class PyrenderScene:
  ...
  def render(self, camera, settings: Settings):
    ...
    image, depth = renderer.render(scene) # type: ignore
...
class WorkspaceRenderer:
  ...
  def colormap_torch(self, depth, near=0.2, far=2.0):
    depth = (depth - near) / (far - near)
    depth = 1 - depth.clamp(0, 1).sqrt()

    return (self.color_map[(255 * depth).to(torch.int)])
  ...
  def render(self, camera, settings: Settings):
    ...
    if settings.view_mode == ViewMode.Depth:
      image_gaussian = self.colormap_torch(depth, near=settings.depth_near, far=settings.depth_far).to(torch.uint8).cpu().numpy()
    else:
